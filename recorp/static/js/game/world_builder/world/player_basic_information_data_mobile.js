...
let hp_percent = `${Math.round((currentPlayer.ship.current_hp * 100) / (currentPlayer.ship.max_hp))}%`;
let move_percent = `${Math.round((currentPlayer.ship.current_movement * 100) / (currentPlayer.ship.max_movement))}%`;
let ap_percent = `${Math.round((currentPlayer.user.current_ap * 100) / (currentPlayer.user.max_ap))}%`;
...
character_statistics_progressbar_hp_text_mobile.textContent = `${currentPlayer.ship.current_hp} / ${currentPlayer.ship.max_hp}`;
character_statistics_progressbar_move_content_mobile.style.width = move_percent;
character_statistics_progressbar_move_text_mobile.textContent = `${currentPlayer.ship.current_movement} / ${currentPlayer.ship.max_movement}`;
character_statistics_progressbar_ap_content_mobile.style.width = ap_percent;
character_statistics_progressbar_ap_text_mobile.textContent = `${currentPlayer.user.current_ap} / ${currentPlayer.user.max_ap}`;
...
let img_src = currentPlayer.user.image == "img.png" ? `/static/img/ux/default-user.svg` : `/static/img/users/${currentPlayer.user.player}/0.gif`

player_modal_container_header_img.src = img_src;
player_modal_container_header_li_name_b.textContent = "Name: ";
player_modal_container_header_li_name_span.textContent = currentPlayer.user.name;
player_modal_container_header_li_archetype_b.textContent = "Archetype: ";
player_modal_container_header_li_archetype_span.textContent = currentPlayer.user.archetype_name;
player_modal_container_header_li_faction_b.textContent = "Faction: ";
player_modal_container_header_li_faction_span.textContent = currentPlayer.faction.name;
...
for (let module_i in currentPlayer.ship.modules) {
    ...
    if (currentPlayer.ship.modules[module_i]["type"].includes("DEFENSE") && !currentPlayer.ship.modules[module_i]["name"].includes('hull')) {
        let defense_name = currentPlayer.ship.modules[module_i]["name"].split(" ")[0].toLowerCase()
        let defense_value = `${Math.round((currentPlayer.ship["current_"+defense_name+"_defense"] * 100) / (currentPlayer.ship.modules[module_i].effect.defense))}%`;
        ...
        defensive_module_div.id = `module-${currentPlayer.ship.modules[module_i]["id"]}`;
        ...
        defensive_module_label.textContent = currentPlayer.ship.modules[module_i]["name"].toLowerCase();
        ...
        defensive_module_text.textContent = `${currentPlayer.ship["current_"+defense_name+"_defense"]} / ${currentPlayer.ship.modules[module_i].effect.defense}`;
    ...
    } else if (currentPlayer.ship.modules[module_i]["type"] == "WEAPONRY") {

        let offensive_module_container = document.createElement('div');
        let offensive_module_name_label = document.createElement('label');

        offensive_module_container.id = `module-${currentPlayer.ship.modules[module_i]["id"]}`;
        offensive_module_container.classList.add('module-container', 'mb-1');
        offensive_module_name_label.textContent = currentPlayer.ship.modules[module_i].name;
        ...
        if ("damage_type" in currentPlayer.ship.modules[module_i]["effect"]) {
            ...
            weapon_damage_type_small_value.textContent = currentPlayer.ship.modules[module_i].effect.damage_type;
            ...
            weapon_damage_small_value.textContent = ` ${currentPlayer.ship.modules[module_i].effect.min_damage}-${currentPlayer.ship.modules[module_i].effect.max_damage}`;
            ...
            weapon_range_small_value.textContent = ` ${currentPlayer.ship.modules[module_i].effect.range}`;
        ...
        electronicWarfare_module_container.id = `module-${currentPlayer.ship.modules[module_i]["id"]}`;
        electronicWarfare_module_container.classList.add('module-container', 'mb-1');
        electronicWarfare_module_name_label.textContent = currentPlayer.ship.modules[module_i].name;
        ...
        other_module_container.id = `module-${currentPlayer.ship.modules[module_i]["id"]}`;
        other_module_container.classList.add('module-container', 'mb-1');
        other_module_name_label.textContent = currentPlayer.ship.modules[module_i].name;
        ...
        for (const [key, value] of Object.entries(currentPlayer.ship.modules[module_i].effect)) {
            ...
            if (typeof value == 'boolean') {
                ...
                if (currentPlayer.ship.modules[module_i]["name"].includes('hull')) {
                    content_value = `+${ value } hp`;
                } else if (currentPlayer.ship.modules[module_i]["name"].includes('propulsion')) {
                    content_value = `+${ value } `;
                } else {
                    content_value = value;
                }
