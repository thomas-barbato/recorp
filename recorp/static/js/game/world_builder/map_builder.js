function append_foreground_menu() {
  let len_element =
    parseInt(document.querySelectorAll(".foreground-menu-item").length) + 1;
  let element = document.querySelector("#foreground-menu-1");
  let clone = element.cloneNode(true);
  clone.id = "foreground-menu-" + len_element;
  let last_element = Array.from(
    document.querySelectorAll(".foreground-menu-item"),
  ).pop();
...
for (let i = 0; i < tile.length; i++) {
  tile[i].addEventListener("click", function () {
    ...
    radio_btn[radio_btn.length - 1].id =
      "coord-radio-button-" + radio_btn.length;
    let id = "";
    for (let i = 0; i < radio_btn.length; i++) {
      if (radio_btn[i].checked) {
        ...
        document.querySelector(
          "#foreground-menu-" +
            id +
            " > div > section.coord > div.coord_x > input",
        ).value = parseInt(value[0]);
        document.querySelector(
          "#foreground-menu-" +
            id +
            " > div > section.coord > div.coord_y > input",
...
function add_background(folder_name) {
  ...
  for (let i = 0; i < game_rows.length; i++) {
    ...
    for (let y = 0; y < cols.length; y++) {
      let bg_url =
        "/static/img/atlas/background/" + folder_name + "/" + cell + ".png";
...
function add_foreground(dict) {
  ...
  for (let data in dict) {
    if (dict[data]["item_select"] !== "none") {
      for (
        let row = dict[data]["coord_y"];
        row < dict[data]["coord_y"] + dict[data]["size_y"];
        row++;
      ) {
        for (
          let col = dict[data]["coord_x"];
          col < dict[data]["coord_x"] + dict[data]["size_x"];
          col++;
        ) {
          let fg_image = document.createElement("img");
          let fg_url =
            "/static/img/atlas/foreground/" +
            dict[data]["item_select"] +
            "/" +
            cell +
            ".png";
          fg_image.classList.add(
            "planete",
            "z-2",
            "absolute",
            "block",
          ...
          fg_image.src = fg_url;
          document
            .querySelector(".tabletop-view")
            .rows[row].cells[col].querySelector("div")
            .append(fg_image);
          cell++;
        }
      }
...
preview.addEventListener("click", function () {
  ...
  for (let i = 0; i < fg_data.length; i++) {
    dict[i] = {
      ...
      size_x: parseInt(
        fg_data[i].querySelector(
          ":scope > div > section.size > div.size_x > input",
        ).value,
      ),
      size_y: parseInt(
        fg_data[i].querySelector(
          ":scope > div > section.size > div.size_y > input",
        ).value,
      ),
      coord_x:
        parseInt(
          fg_data[i].querySelector(
            ":scope > div > section.coord > div.coord_x > input",
          ).value,
        ) + 1,
      coord_y:
        parseInt(
          fg_data[i].querySelector(
            ":scope > div > section.coord > div.coord_y > input",
          ).value,
        ) + 1,
    };
  }
