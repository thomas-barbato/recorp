...
class GameConsumer(WebsocketConsumer):
    """
    WebSocket consumer avec optimisations conservatrices.
    Préserve la logique métier originale tout en améliorant les performances.
    """
    ...
    def _initialize_attributes(self) -> None:
        ...
        # Cache des objets coûteux (optimisation sûre)
    ...
    def _setup_room_connection(self) -> None:
        ...
        # Cache des objets pour éviter les re-créations
        if not self._player_action:
            self._player_action = PlayerAction(self.user.id)
        self.player_id = self._player_action.get_player_id()
    ...
    def _handle_authenticated_user(self) -> None:
        ...
        if self.user.is_authenticated and self._cache_store:
            self._cache_store.get_or_set_cache(need_to_be_recreated=False)

    def disconnect(self, close_code: int) -> None:
        """Gère la déconnexion WebSocket avec nettoyage."""
        try:
    ...
    def async_move(self, event: Dict[str, Any]) -> None:
        """
        LOGIQUE ORIGINALE PRÉSERVÉE avec seulement optimisations d'erreurs.
        """
        try:
            message = json.loads(event["message"])
            
            # Utilisation des objets cachés si disponibles, sinon création
            player_action = self._player_action or PlayerAction(self.user.id)
            
            # LOGIQUE ORIGINALE : Traitement du mouvement
            if self._can_move_to_destination(player_action, message):
                if self._register_move(player_action, message):
                    # IMPORTANT: Utiliser l'objet cache existant ou en créer un nouveau
                    cache_store = self._cache_store or StoreInCache(self.room_group_name, self.user)
                    
                    cache_store.update_player_position(message, self.player_id)
                    # Toujours mettre à jour la portée du joueur connecté
                    cache_store.update_player_range_finding()
                    
                    # Préparer la réponse selon le type de mouvement (LOGIQUE ORIGINALE)
                    if self._is_own_player_move(self.player_id, message):
                        # Le joueur connecté a bougé
                        cache_store.update_sector_player_visibility_zone(self.player_id)
                        response = self._create_own_move_response(message, cache_store, self.player_id)
                    else:
                        # Un autre joueur a bougé
                        response = self._handle_other_player_move(message, player_action, cache_store, self.player_id)
            ...
            logger.error(f"Erreur inattendue dans async_move: {e}")
    ...
    def _create_own_move_response(
        ...
        """Crée la réponse pour le mouvement du propre joueur. LOGIQUE ORIGINALE."""
        return {
            "type": "player_move",
            "message": {
                "player_id": message["player"],
    ...
    def _handle_other_player_move(
        ...
        """Gère le mouvement d'un autre joueur. LOGIQUE ORIGINALE."""
    ...
    def async_reverse_ship(self, event: Dict[str, Any]) -> None:
        """
        Gère l'inversion asynchrone du vaisseau. LOGIQUE ORIGINALE PRÉSERVÉE.
        """
        try:
            message = json.loads(event["message"])
            ...
            logger.error(f"Erreur inattendue dans async_reverse_ship: {e}")

    def _process_ship_reversal(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """Traite l'inversion du vaisseau. LOGIQUE ORIGINALE."""
        player_action = self._player_action or PlayerAction(self.user.id)
        cache_store = self._cache_store or StoreInCache(room_name=self.room_group_name, user_calling=self.user)
        
        player_action.set_reverse_ship_status()
        data = cache_store.update_ship_is_reversed(
    ...
    def async_warp_travel(self, event: Dict[str, Any]) -> None:
        """
        Gère le voyage par distorsion asynchrone. LOGIQUE ORIGINALE PRÉSERVÉE.
        """
        try:
            ...
            logger.error(f"Erreur inattendue dans async_warp_travel: {e}")

    def _process_warp_travel(self, warp_data: Dict[str, Any]) -> None:
        """Traite le voyage par distorsion. LOGIQUE ORIGINALE PRÉSERVÉE."""
    ...
    def _notify_destination_room(self, destination_room_key: str, player_id: int) -> None:
        ...
        try:
            ...
            logger.error(f"Erreur lors de la notification de destination: {e}")
